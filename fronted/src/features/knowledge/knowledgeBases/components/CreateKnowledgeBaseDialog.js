    ...
    >
      <div style={{ width: 'min(680px, 95vw)', background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
        <div style={{ padding: '12px 14px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontWeight: 800 }}>Create Knowledge Base</div>
        ...
        <div style={{ padding: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <label>鍚嶇О</label>
            <input data-testid="kbs-create-name" value={createName} onChange={(event) => onCreateNameChange && onCreateNameChange(event.target.value)} style={{ padding: '9px 10px', border: '1px solid #d1d5db', borderRadius: 8 }} />
          </div>
        ...
        <div style={{ padding: '12px 14px', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button type="button" onClick={onClose} style={{ border: '1px solid #d1d5db', borderRadius: 8, background: '#fff', cursor: 'pointer', padding: '8px 12px' }}>鍙栨秷</button>
          <button type="button" data-testid="kbs-create-submit" onClick={onCreate} disabled={!isAdmin || kbBusy} style={{ border: '1px solid #2563eb', borderRadius: 8, background: kbBusy ? '#93c5fd' : '#2563eb', color: '#fff', cursor: kbBusy ? 'not-allowed' : 'pointer', padding: '8px 12px' }}>鍒涘缓</button>
        </div>
      </div>
    </div>
  );
