...
export default function MaintenanceManagement() {
  ...
  const loadRecords = useCallback(async () => {
    ...
    setError('');
    try {
      const next = await maintenanceApi.listRecords({ limit: 100 });
      setRecords(next);
    } catch (err) {
      setError(err.message || '加载维护记录失败');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleCreate = useCallback(async () => {
    ...
    setSuccess('');
    try {
      await maintenanceApi.createRecord(createPayload);
      setCreateForm(initialForm);
      setSuccess('维护记录已创建');
      await loadRecords();
    } catch (err) {
      setError(err.message || '创建维护记录失败');
    } finally {
      setSaving(false);
    }
  ...
  const handleAction = useCallback(
    async (item, action) => {
      ...
      try {
        if (action === 'record') {
          await maintenanceApi.recordExecution(item.record_id, {
            performed_at_ms: Date.now(),
            summary: item.summary || '维护完成',
            next_due_date: item.next_due_date || item.planned_due_date,
          });
        } else if (action === 'approve') {
          await maintenanceApi.approveRecord(item.record_id, { notes: '审批通过' });
        }
        setSuccess(`记录已处理: ${action}`);
        await loadRecords();
      } catch (err) {
        setError(err.message || '维护记录处理失败');
      }
    },
    [loadRecords]
  );
  ...
  const handleDispatch = useCallback(async () => {
    ...
    setSuccess('');
    try {
      const result = await maintenanceApi.dispatchReminders(7);
  ...
  return (
    ...
    >
      ...
      {error ? (
        <div data-testid="maintenance-error" style={{ ...panelStyle, color: '#9f1239' }}>
          {error}
        </div>
      ) : null}
      {success ? (
        <div data-testid="maintenance-success" style={{ ...panelStyle, color: '#166534' }}>
          {success}
        </div>
      ) : null}

      <section style={panelStyle}>
        <h3 style={{ marginTop: 0 }}>新建维护记录</h3>
        <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
          <input
            data-testid="maintenance-create-equipment-id"
            style={inputStyle}
            placeholder="设备 ID"
          ...
          />
          <input
            data-testid="maintenance-create-responsible-user-id"
            style={inputStyle}
          ...
          />
          <select
            data-testid="maintenance-create-type"
            style={inputStyle}
            value={createForm.maintenance_type}
            onChange={(event) =>
              setCreateForm((p) => ({ ...p, maintenance_type: event.target.value }))
            }
          >
          ...
          <input
            data-testid="maintenance-create-planned-due-date"
            style={inputStyle}
          ...
          />
          <input
            data-testid="maintenance-create-summary"
            style={inputStyle}
            placeholder="摘要"
      ...
      <section style={panelStyle}>
        <h3 style={{ marginTop: 0 }}>维护记录列表</h3>
        {loading ? <div data-testid="maintenance-loading">加载中...</div> : null}
        {!loading && records.length === 0 ? <div data-testid="maintenance-empty">暂无维护记录</div> : null}
        <div style={{ display: 'grid', gap: 10 }}>
          {records.map((item) => (
            <div
              key={item.record_id}
              data-testid={`maintenance-row-${item.record_id}`}
            ...
            >
              ...
              <div>类型: {item.maintenance_type}</div>
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                {(STATUS_ACTIONS[item.status] || []).map((action) => (
                  <button
                    key={action}
                    type="button"
                    data-testid={`maintenance-action-${item.record_id}-${action}`}
                    style={buttonStyle}
