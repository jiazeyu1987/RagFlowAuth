...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate GBZ-01 repository compliance state.")
    parser.add_argument("--root", default=str(REPO_ROOT), help="Repository root path")
    parser.add_argument("--json", action="store_true", help="Print JSON report")
    args = parser.parse_args()

    report = validate_gbz01_repo_state(args.root)
    if args.json:
        print(json.dumps(report.as_dict(), ensure_ascii=False, indent=2))
    else:
        print(f"GBZ-01 repository gate: {'PASS' if report.passed else 'FAIL'}")
