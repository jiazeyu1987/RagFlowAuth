...
class TestReleasePublishLocalToTestUnit(unittest.TestCase):
    def test_ssh_timeout_returns_error_tuple(self) -> None:
        with patch.object(
            release_publish_local_to_test.subprocess,
            "run",
            side_effect=subprocess.TimeoutExpired(cmd=["ssh"], timeout=60),
        ):
            ok, out = release_publish_local_to_test._ssh("172.30.30.58", "echo OK", timeout_s=60)

        self.assertFalse(ok)
        self.assertIn("Command timed out after 60 seconds", out)
    ...
    def test_local_to_test_cleanup_timeout_does_not_abort_publish(self) -> None:
        tmp_root = make_temp_dir(prefix="ragflowauth_ut")
        ...
        def fake_ssh(ip: str, command: str, *, timeout_s: int = 60):
            if "rm -f /tmp/ragflowauth_release_" in command:
        ...
        with patch.object(release_publish_local_to_test, "_run_local", side_effect=fake_run_local), patch.object(
            ...
            res = release_publish_local_to_test.publish_from_local_to_test(version="v123")
            self.assertTrue(res.ok, res.log)
            self.assertIn("[STAGING] [WARN] cleanup legacy /tmp release artifacts failed", res.log)
