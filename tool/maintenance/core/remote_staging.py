...
class RemoteStagingManager:
    ...
    def _exec_best_effort(self, command: str, *, action: str) -> None:
        try:
            ok, out = self._exec(command)
        except Exception as exc:
            self._log_line(f"[STAGING] [WARN] {action} failed: {exc}")
            return
        if not ok:
            reason = (out or "").strip() or "unknown error"
            self._log_line(f"[STAGING] [WARN] {action} failed: {reason}")
    ...
    def cleanup_legacy_tmp_release_files(self) -> None:
        self._exec_best_effort(
            ...
            action="cleanup legacy /tmp release artifacts",
        )

    def cleanup_path(self, path: str) -> None:
        self._exec_best_effort(f"rm -f {path} 2>/dev/null || true", action=f"cleanup staging path {path}")
