...
set "TOOL_PY=%ROOT%tool\maintenance\server_backup_pull_tool.py"

if not exist "%TOOL_PY%" (
  echo [ERROR] server_backup_pull_tool.py not found: %TOOL_PY%
  pause
  exit /b 1
)
...
if not "%CODE%"=="0" (
  echo [ERROR] server_backup_pull_tool.py exited with code %CODE%
  pause
)
