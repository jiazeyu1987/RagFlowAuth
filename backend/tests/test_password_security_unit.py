...
class TestPasswordSecurityUnit(unittest.TestCase):
    ...
    def test_password_history_and_credential_lockout_are_persisted(self):
        ...
        try:
            ...
            locked_until = None
            for _ in range(5):
                locked_until = store.record_credential_failure(user.user_id, now_ms=1_000)
    ...
    def test_signature_challenge_upgrades_legacy_hash_and_locks_after_failures(self):
        ...
        try:
            ...
            challenge = service.issue_challenge(user=user, password=SIGN_PASSWORD, user_store=user_store)
            ...
            for _ in range(5):
                try:
                    service.issue_challenge(user=upgraded_user, password="WrongPass123", user_store=user_store)
