...
""",
    ...
    _write(root / "backend/app/modules/review/routes/approve.py", "assert_user_training_for_action\n")
    _write(root / "backend/app/modules/review/routes/reject.py", "assert_user_training_for_action\n")
    _write(root / "backend/app/modules/review/routes/overwrite.py", "assert_user_training_for_action\n")
...
class TestGbz05ComplianceGateUnit(unittest.TestCase):
    ...
    def test_repo_gate_flags_missing_runtime_gate(self):
        ...
        try:
            root = Path(tmp)
            _build_repo(root)
            (root / "backend/app/modules/review/routes/approve.py").write_text("# missing gate\n", encoding="utf-8")
            report = validate_gbz05_repo_state(root, as_of=date(2026, 4, 3))
