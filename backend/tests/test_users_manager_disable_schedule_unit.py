...
class UserManagementManagerDisableScheduleTests(unittest.TestCase):
    def setUp(self):
        self.port = _FakePort()
        self.manager = UserManagementManager(self.port)

    def test_viewer_can_be_disabled_until_future(self):
        until_ms = int(time.time() * 1000) + 60_000
        result = self.manager.update_user(
            user_id="u_viewer",
            user_data=UserUpdate(disable_login_enabled=True, disable_login_until_ms=until_ms),
        )
        self.assertTrue(result.disable_login_enabled)
        self.assertEqual(until_ms, result.disable_login_until_ms)
        self.assertTrue(result.login_disabled)

    def test_disable_until_must_be_future(self):
        past_ms = int(time.time() * 1000) - 1
        with self.assertRaises(UserManagementError) as ctx:
            self.manager.update_user(
                user_id="u_viewer",
                user_data=UserUpdate(disable_login_enabled=True, disable_login_until_ms=past_ms),
            )
        self.assertEqual("disable_login_until_must_be_future", ctx.exception.code)

    def test_builtin_admin_cannot_use_disable_schedule(self):
        until_ms = int(time.time() * 1000) + 60_000
        with self.assertRaises(UserManagementError) as ctx:
            self.manager.update_user(
                user_id="u_admin",
