...
class PaperDownloadManager:
    ...
    def _normalize_source_configs(source_configs: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
        return DownloadExecutionManager(namespace="paper_download").normalize_source_configs(
    ...
    def _build_source_stats(enabled: list[str], cfg: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
        return DownloadExecutionManager(namespace="paper_download").build_source_stats(
        ...
        *,
        ...
        try:
            ...
            if isinstance(e, HTTPException):
                if not str(e.detail).startswith("paper_add_to_kb_failed:"):
                    raise HTTPException(status_code=e.status_code, detail=f"paper_add_to_kb_failed: {e.detail}") from e
                raise
    ...
    def delete_item(self, *, session_id: str, item_id: int, ctx: Any, delete_local_kb: bool = True) -> dict[str, Any]:
        return self._kb_lifecycle_manager.delete_item(
            ...
            not_found_detail="paper_item_not_found",
            action_name="paper_item_delete",
            source_name="paper_download",
        )
