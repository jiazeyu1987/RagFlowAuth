...
MAINTENANCE_STATUSES = {"planned", "recorded", "approved"}
...
class MaintenanceService:
    ...
    def _require_text(value: Any, field_name: str) -> str:
        ...
        if not text:
            raise MaintenanceServiceError(f"{field_name}_required", status_code=400)
        return text
    ...
    def _optional_iso_date(value: Any, *, field_name: str) -> str | None:
        ...
        try:
            ...
            raise MaintenanceServiceError(f"invalid_{field_name}", status_code=400) from exc
        return text
    ...
    def _optional_timestamp_ms(value: Any, *, field_name: str) -> int | None:
        ...
        try:
            ...
            raise MaintenanceServiceError(f"invalid_{field_name}", status_code=400) from exc
        if parsed <= 0:
            raise MaintenanceServiceError(f"invalid_{field_name}", status_code=400)
        return parsed
    ...
    def _require_equipment_row(self, conn, equipment_id: str):
        ...
        if row is None:
            raise MaintenanceServiceError("equipment_asset_not_found", status_code=404)
        return row
    ...
    def _require_record_row(self, conn, record_id: str):
        ...
        if row is None:
            raise MaintenanceServiceError("maintenance_record_not_found", status_code=404)
        return row
        ...
        *,
        ...
        for item in attachments or []:
            if not isinstance(item, dict):
                raise MaintenanceServiceError("invalid_attachments", status_code=400)
            ...
            if uploaded_at_raw is None or str(uploaded_at_raw).strip() == "":
                ...
                try:
                    ...
                    raise MaintenanceServiceError("invalid_attachments", status_code=400) from exc
            normalized.append(
                {
    ...
    def _serialize_record(row) -> dict[str, Any]:
        return {
            ...
            "outcome_summary": (str(row["outcome_summary"]) if row["outcome_summary"] else None),
    ...
    def list_records(
        ...
        *,
        ...
        if status:
            clean_status = self._require_known_value(status, field_name="status", allowed=MAINTENANCE_STATUSES)
            where.append("status = ?")
        ...
        *,
        ...
        try:
            ...
            if str(equipment_row["status"]) == "retired":
                raise MaintenanceServiceError("equipment_asset_retired", status_code=409)
            conn.execute(
                """
                ...
                (
                    ...
                    performed_at_ms,
                    summary,
                    self._optional_text(outcome_summary),
                    next_due_date,
            ...
            if status == "recorded":
                conn.execute(
                    ...
                    SET status = 'under_maintenance', updated_by_user_id = ?, updated_at_ms = ?
                    WHERE equipment_id = ?
                    """,
    ...
    def record_execution(
        ...
        *,
        ...
        if performed_at_ms is None:
            raise MaintenanceServiceError("performed_at_ms_required", status_code=400)
        summary = self._require_text(summary, "summary")
        next_due_date = self._optional_iso_date(next_due_date, field_name="next_due_date")
        now_ms = int(time.time() * 1000)
        clean_attachments = self._normalize_attachments(
        ...
        try:
            ...
            if str(row["status"]) not in {"planned", "recorded"}:
                raise MaintenanceServiceError("maintenance_record_invalid_state", status_code=409)
            conn.execute(
                """
        ...
        *,
        ...
        try:
            ...
            if str(row["status"]) != "recorded":
                raise MaintenanceServiceError("maintenance_record_must_be_recorded", status_code=409)
            conn.execute(
                """
                UPDATE maintenance_records
                SET status = 'approved',
    ...
    def dispatch_due_reminders(self, *, actor_user_id: str, window_days: int = 7) -> dict[str, Any]:
        ...
        if self._notification_manager is None:
            raise MaintenanceServiceError("maintenance_notification_manager_unavailable", status_code=500)
        try:
            normalized_window_days = max(1, min(int(window_days or 7), 365))
        except Exception as exc:
            raise MaintenanceServiceError("invalid_window_days", status_code=400) from exc
        today = date.today()
        ...
        for row in rows:
            ...
            if str(row["reminder_sent_for_due_date"] or "") == due_date_text:
                continue
            payload = {
                "event_type": "maintenance_due_soon",
                "title": f"维护临期提醒：{row['equipment_id']}",
                "body": f"设备 {row['equipment_id']} 的维护节点将于 {due_date_text} 到期，请责任人处理。",
            ...
            jobs = self._notification_manager.notify_event(
                ...
                dedupe_key=f"maintenance_due_soon:{row['record_id']}:{due_date_text}",
