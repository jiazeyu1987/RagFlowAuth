...
def _approve_document_impl(
    ...
    signature_action = "document_approve" if bool(progress.get("is_final_step")) else "document_approve_step"
    ...
    # Intermediate approval step: move to next step, keep pending.
    if not bool(progress.get("is_final_step")):
        try:
            ...
            raise HTTPException(status_code=e.status_code, detail=e.code) from e
        after_state = _doc_audit_state(doc, approval=approval)
        try:
        ...
        deps.audit_log_manager.log_record_change(
            ...
            doc_id=doc.doc_id,
            filename=doc.filename,
            kb_id=(getattr(doc, "kb_name", None) or doc.kb_id),
            kb_dataset_id=getattr(doc, "kb_dataset_id", None),
            kb_name=getattr(doc, "kb_name", None) or doc.kb_id,
            meta={"action": "approve_step", "signature_meaning": signature_meaning},
        )
        return _to_document_response(doc, approval=approval, signature=signature)

    # Final approval step: keep existing upload/parse/approve flow.
    try:
        ...
        deps.audit_log_manager.log_record_change(
            ...
            meta={"action": "approve", "signature_meaning": signature_meaning},
        )
...
def approve_documents_batch(body: BatchDocumentReviewRequest, request: Request, ctx: AuthContextDep):
...
def approve_document(
    ...
    return _approve_document_impl(doc_id, ctx, request=request, review_data=review_data)
