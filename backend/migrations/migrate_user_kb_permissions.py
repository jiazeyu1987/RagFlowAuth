...
执行方式：
    python -m backend.migrations.migrate_user_kb_permissions
...
def _resolve_db_path(raw: str | None) -> str:
    return str(resolve_auth_db_path(raw))
...
def migrate_user_permissions(db_path: str = None):
    ...
    user_store = UserStore(db_path=resolved_db_path)
    permission_store = UserKbPermissionStore(db_path=resolved_db_path)
    ...
    try:
        datasets = ragflow_service.list_all_datasets()
        all_kbs = datasets if datasets else [{"id": None, "name": "展厅"}]
        print(f"      找到 {len(all_kbs)} 个知识库: {', '.join([kb['name'] for kb in all_kbs])}")
    except Exception as e:
        print(f"      警告: 无法连接RAGFlow，使用默认知识库列表 ['展厅']")
        all_kbs = [{"id": None, "name": "展厅"}]
        ...
        if user.role == "admin":
            ...
            for kb in all_kbs:
                permission_store.grant_permission(
                    user.user_id,
                    kb.get("id") or kb.get("name") or "展厅",
                    system_user_id,
                    kb_dataset_id=kb.get("id"),
                    kb_name=kb.get("name") or "展厅",
                )
                stats["total_permissions"] += 1
            stats["admin"] += 1
            print(f"      [管理员] {user.username} -> 所有知识库 ({len(all_kbs)}个)")

        elif user.role in ["reviewer", "operator", "viewer"]:
            # 审核员、操作员、查看者：授予默认知识库权限
            default_kb = "展厅"
            permission_store.grant_permission(user.user_id, default_kb, system_user_id, kb_name=default_kb)
            stats["total_permissions"] += 1
            stats[user.role] += 1
